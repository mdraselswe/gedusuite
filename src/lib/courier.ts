...
export type CourierQuoteInput = {
  /** The zone's flat rate up to baseWeightKg. */
  zoneRate: number;
 ...
 */
function weightCharge(rules: CourierRules, weightKg: number | null | undefined): number {
  if (weightKg == null) return 0;
  const over = weightKg - rules.baseWeightKg;
  if (over <= 0) return 0;
...
export function quoteCourier(rules: CourierRules, input: CourierQuoteInput): CourierQuote {
  const extra = weightCharge(rules, input.weightKg);
  const deliveryCharge = round2(input.zoneRate + extra);
  ...
  return {
    zoneRate: round2(input.zoneRate),
    weightCharge: extra,
    deliveryCharge,
    codFee,
...
export function quoteReturnCharge(
  rules: CourierRules,
  input: { zoneRate: number; weightKg?: number | null },
): number {
  const delivery = round2(input.zoneRate + weightCharge(rules, input.weightKg));
...
export function breakEvenDeliveryCharge(
  rules: CourierRules,
  input: { zoneRate: number; weightKg?: number | null; goodsAmount: number },
): number | null {
  const delivery = round2(input.zoneRate + weightCharge(rules, input.weightKg));
